...
$the_url = $get_action == 'new_notebook' ? 'child.feed':'apoderado.notes.show';
?>
<a href=" {{route($the_url,['id'=>$notification->data['action'],'nid'=>'?nid='.$notification->id.''])}} " class="d-block notification-link p-2 border-bottom  {{$notification->read_at ? '':'is-active'}}">
  <div class="d-flex flex-row mb-0 pt-2 pb-2 notification-item" id="notification-item" data-notification="{{$notification->id}}" >
    ...
    <div class="pl-3 pr-2">
      <p class="font-weight-medium mb-1">
        ...
        @else
        Te envió una nuava circular! <strong>{{ $notification->data['message'] }}
        </strong>
        @endif
      </p>
